...
export class Mint2Component implements OnInit {
      ...
      this.trust
        .changeTrustByDistributor(
          ...
          () => {
            this.flag = false;
            this.snackbar.openSnackBar(
              'Transaction Failed: User closed Wallet',
              'error'
            );
            this.pendingDialog.close(false);
          }
  ...
  ngOnChanges(): void {
    ...
    this.controlGroup = new FormGroup({
      ...
      Currency: new FormControl(this.mint.Currency, Validators.required),
  ...
  onTagInputChange() {
    if (this.tagInputText.charAt(this.tagInputText.length - 1) === ',') {
      this.addAfterComma(
